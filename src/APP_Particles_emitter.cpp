...
void APP_particleEmitter::initalise(unsigned int a_maxParticles,
	unsigned int a_emitRate,
	float a_lifetimeMin, float a_lifetimeMax,
	float a_velocityMin, float a_velocityMax,
	float a_startSize, float a_endSize,
	const glm::vec4& a_startColour, const glm::vec4& a_endColour, 
	unsigned int* a_vao, unsigned int* a_vbo, unsigned int* a_ibo, unsigned int a_progid) {
	// set up emit timers
	m_emitTimer = 0;
...
void APP_particleEmitter::update(float a_deltaTime,
...
}void APP_particleEmitter::draw(unsigned int a_vao, unsigned int a_vbo, unsigned int a_progid,
	Camera* a_cam) 
{
...
//	glUseProgram(m_programID);
	int view_proj_uniform = glGetUniformLocation(a_progid, "projectionView");
	glUniformMatrix4fv(view_proj_uniform, 1, GL_FALSE, glm::value_ptr(a_cam->GetProjectionView()));
